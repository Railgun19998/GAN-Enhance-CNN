...
# 散点图
    if sw == 1:
        ...
        scr = ax.scatter(prediction1, experiment1, 
                ...
                s=60,
                marker='o',
                label=f'Train Data points (n={len(prediction1)})')
        scs = ax.scatter(prediction2, experiment2, 
                ...
                s=60,
                marker='s',
                label=f'Tast Data points (n={len(prediction2)})')
    if sw == 0:
...
# 标签与图例
    ...
    ax.set_title('GAN Generator Enhance CNN Prediction', fontsize=14, pad=20)
    ax.legend(loc='upper left', framealpha=1)
    plt.tight_layout()

# 保存图像
    output_path = f"GAN_Enh_plot.png"  
    plt.savefig(output_path, dpi=300, bbox_inches='tight') 
    print(f"图像已保存为 {output_path}")
    plt.show()

data = pd.read_csv("data.S3.csv",encoding='ANSI')
...
checkpoint = ModelCheckpoint(filepath="cnn_Ti1.h5", monitor='val_loss', mode='min', save_best_only=True, verbose=1)
# model.fit(x_train, y_train, epochs=1000, batch_size=8,callbacks=[checkpoint],validation_data=[x_test,y_test])

m = load_model("cnn_Ti1.h5")
pre_test = m.predict(x_test).flatten()
pre_train = m.predict(x_train).flatten()

printPlt(pre_train,y_train,pre_test,y_test,0)
